...
#echo = true wanna see code, warning and messages dont show up
#janitor good for data cleaning and more, janirotr clean names takes names and puts into lowercasesnakecase
...
```{r}
#toolik_weather <- read_csv(here::here("data", "data-raw", "toolik_weather.csv")) dont need double colon but explicity says get fucntion from this package
#here always starts at root of working drectiry here()
#without pipe you dont have t oout things into parentheses 
# %>% or |> for pipeling 
#cleaan_names goes to lower snake casae immediately 
#within csv but outsdie where were pointing to file 
#na = ... changes NA to actual NA rather than actual variables 
...
```{r}
#jitterplot, if scatter it would be stacked 
# columns are vectors, can only be single type
ggplot(data = toolik_weather, (aes(x = 
```
...
ggsave(here::here("figs", "myAwesomeChart.jpg), plot = myAwesomeChart, height = 5, width = 6"))
...
myAwesomeChart <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c)) +
  ...
  scale_color_gradient(colors = c("red", "yellow", "purple"))

#?ggsave() goes to summarya bout funtion, need here to give it path, here starts at working directory, type here and it tells you where starting point is, path i need you to take is figs and give a file name

view(toolik_temp_plot)
...
ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c)) +
  geom_jitter(aes(colour = daily_air_temp_mean_c)) +
  scale_colour_gradient(low = "red", high = "blue"))
```
```{r}
#heere in other direction
